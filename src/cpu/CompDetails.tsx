...
export const CompDetails: React.FC<{
    ...
    function handleInternalSchematicAddNew() {
        let newSchematic = editorState.schematicLibrary.addCustomSchematic('Nuevo Esquemático')!;
    ...
    return <div className="flex flex-col flex-1">
        {numSelected === 0 && <div>Ningún componente seleccionado</div>}
        {numSelected === 1 && singleComp && compDef && <div>
            ...
            <div className="mx-2">
                ...
                <EditKvp label={'Id Def'}><code>{singleComp.defId}</code></EditKvp>
                <EditKvp label={'Nombre'}><StringEditor className="bg-slate-100 rounded flex-1" value={singleComp.name} update={handleNameUpdate} /></EditKvp>
                <EditKvp label={'Id Ext'}><StringEditor className="bg-slate-100 rounded font-mono flex-1" value={singleComp.extId ?? ''} update={handleExtIdUpdate} /></EditKvp>
                <EditKvp label={'Pos'}>{`${singleComp.pos}`}</EditKvp>
                <EditKvp label={'Tamaño'}>{`${singleComp.size}`}</EditKvp>
            </div>
            <div className="m-2">
                <div className="mb-2">Esquemático Interno</div>
                {!subSchematic && <button
                ...
                {subSchematic && <EditKvp label={'Nombre'}><code>{subSchematic?.name}</code></EditKvp>}
            </div>
        </div>}
    </div>;
};
