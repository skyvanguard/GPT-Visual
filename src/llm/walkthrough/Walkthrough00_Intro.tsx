...
export function walkthroughIntro(args: IWalkthroughArgs) {
    ...
    let c0 = commentary(wt, null, 0)`¡Bienvenido al recorrido guiado del modelo de lenguaje grande GPT! Aquí exploraremos el modelo _nano-gpt_, con apenas 85,000 parámetros.

Su objetivo es simple: tomar una secuencia de seis letras: ${embed(ExampleInputOutput)}
y ordenarlas alfabéticamente, es decir, a "ABBBCC".`;
    ...
    let tokenIdxStr = c_str('_índice de token_', 0, DimStyle.TokenIdx);

    commentary(wt, t6)`A cada una de estas letras la llamamos ${tokenStr}, y el conjunto de los diferentes tokens del modelo forma su _vocabulario_:${embed(TokenVocab)}

    De esta tabla, a cada token se le asigna un número, su ${tokenIdxStr}. Y ahora podemos ingresar esta secuencia de números al modelo:${embed(ExampleTokenValues)}\n`;
    breakAfter();
    ...
    let c5 = commentary(wt)`En la vista 3D, cada celda verde representa un número siendo procesado, y cada celda azul es un peso. ${embed(GreenBlueCells)}
    Cada número en la secuencia primero se convierte en un vector de 48 elementos (un tamaño elegido para este modelo en particular). Esto se llama _embedding_.`;
    breakAfter(c5);
    ...
    breakAfter();
    commentary(wt)`El embedding luego pasa a través del modelo, atravesando una serie de capas llamadas transformers, antes de llegar al final.`;
    breakAfter();
    ...
    commentary(wt)`¿Cuál es la salida entonces? Una predicción del siguiente token en la secuencia. Así que en la 6ª entrada, obtenemos probabilidades de que el siguiente token
        sea 'A', 'B', o 'C'.`

    commentary(wt)`En este caso, el modelo está bastante seguro de que será 'A'. Ahora, podemos alimentar esta predicción de vuelta a la parte superior del modelo, y repetir
    todo el proceso.`;
        ...
        <table className={s.table}>
            <tbody>
                ...
                <tr className={s.tokIndex} style={{ color: dimStyleColor(DimStyle.TokenIdx).toHexColor() }}>
                    <th>índice</th><td>0</td><td>1</td><td>2</td>
                </tr>
            </tbody>
        </table>
    </div>
