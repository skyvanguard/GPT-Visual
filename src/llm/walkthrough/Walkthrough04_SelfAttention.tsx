...
las columnas pueden considerarse independientemente.
...
La capa de auto-atención está compuesta por varias cabezas, y nos enfocaremos en una de ellas por ahora.`;
    ...
    breakAfter();
    commentary(wt)`
El primer paso es producir tres vectores para cada una de las ${c_dimRef('T', DimStyle.T)} columnas de la ${c_blockRef('matriz de embedding de entrada normalizada', block0.ln1.lnResid)}.
Estos vectores son los vectores Q, K y V:

${embedInline(<ul>
    <li>Q: <BlockText blk={head2.qBlock}>Vector de consulta (Query)</BlockText></li>
    <li>K: <BlockText blk={head2.kBlock}>Vector de clave (Key)</BlockText></li>
    <li>V: <BlockText blk={head2.vBlock}>Vector de valor (Value)</BlockText></li>
</ul>)}

Para producir uno de estos vectores, realizamos una multiplicación matriz-vector con un sesgo añadido. Cada
celda de salida es alguna combinación lineal del vector de entrada. Por ejemplo, para los ${c_blockRef('vectores Q', head2.qBlock)}, esto se hace con un producto punto entre
una fila de la ${c_blockRef('matriz de pesos Q', head2.qWeightBlock)} y una columna de la ${c_blockRef('matriz de entrada', block0.ln1.lnResid)}.`;
    breakAfter();
...
primer vector con el elemento correspondiente del segundo vector, multiplicamos los pares
...
Esta es una forma general y simple de asegurar que cada elemento de salida pueda ser influenciado por todos los
elementos en el vector de entrada (donde esa influencia es determinada por los pesos). De ahí su frecuente
aparición en redes neuronales.
...
Repetimos esta operación para cada celda de salida en los vectores Q, K, V:`;
    ...
    breakAfter();
    commentary(wt)`
¿Qué hacemos con nuestros vectores Q (consulta), K (clave) y V (valor)? El nombre
...
${embedInline(<div className='ml-4'>
    <div className='mt-1 text-center italic'>Analogía con software</div>
    <div className='text-sm mt-1 mb-1 text-gray-600'>Tabla de búsqueda:</div>
    <div className='font-mono'>{'tabla = { "clave0": "valor0", "clave1": "valor1", ... }'}</div>
    <div className='text-sm mt-1 mb-1 text-gray-600'>Proceso de consulta:</div>
    <div className='font-mono'>{'tabla["clave1"] => "valor1"'}</div>
</div>)}

En el caso de auto-atención, en lugar de devolver una sola entrada, devolvemos alguna combinación
ponderada de las entradas. Para encontrar esa ponderación, tomamos un producto punto entre un vector Q y cada
uno de los vectores K. Normalizamos esa ponderación, antes de finalmente usarla para multiplicar con el
vector V correspondiente, y luego sumamos todos.
...
${embedInline((() => {
    ...
    return <div className='ml-4'>
        ...
        <div className='text-sm mt-2 mb-1 text-gray-600'>Tabla de búsqueda:</div>
        ...
        <div className='text-sm mt-2 mb-1 text-gray-600'>Proceso de consulta:</div>
        ...
        <div className='font-mono flex items-center my-2'>
            [w0n, w1n, w2n] =&nbsp;<span className='italic'>normalización</span>([w0, w1, w2])
        </div>
        <div className='font-mono flex items-center'>
            resultado =
...
Para un ejemplo más concreto, veamos la 6ª columna (${c_dimRef('t = 5', DimStyle.T)}), desde la cual
haremos la consulta:`;
    breakAfter();
...
Primero calculamos el producto punto entre el ${c_blockRef('vector Q', head2.qBlock)} de la columna actual (${c_dimRef('t = 5', DimStyle.T)}) y los ${c_blockRef('vectores K', head2.kBlock)}
de cada una de esas columnas anteriores. Estos se almacenan luego en la fila correspondiente (${c_dimRef('t = 5', DimStyle.T)})
...
los tokens no pueden "ver el futuro".

Otro elemento es que después de tomar el producto punto, dividimos por sqrt(${c_dimRef('A', DimStyle.A)}), donde
${c_dimRef('A', DimStyle.A)} es la longitud de los vectores Q/K/V. Este escalado se hace para evitar que valores grandes
dominen la normalización (softmax) en el siguiente paso.

Omitiremos mayormente la operación softmax (descrita después); basta decir que cada fila se normaliza para sumar
1.`;
    breakAfter();
    ...
    breakAfter();
    commentary(wt)`
Finalmente, podemos producir el vector de salida para nuestra columna (${c_dimRef('t = 5', DimStyle.T)}). Miramos la fila (${c_dimRef('t = 5', DimStyle.T)}) de la
${c_blockRef('matriz de auto-atención normalizada', head2.attnMtxSm)} y para cada elemento, multiplicamos el ${c_blockRef('vector V', head2.vBlock)} correspondiente de las
otras columnas elemento por elemento.`;
    ...
    breakAfter();
    commentary(wt)`
Luego podemos sumarlos para producir el vector de salida. Así, el vector de salida estará dominado por
...
Ahora que conocemos el proceso, ejecutémoslo para todas las columnas.`;
    ...
    breakAfter();
    commentary(wt)`
Y ese es el proceso para una cabeza de la capa de auto-atención. Así que el objetivo principal de la auto-atención es
que cada columna quiere encontrar información relevante de otras columnas y extraer sus valores, y
lo hace comparando su vector de _consulta_ con las _claves_ de esas otras columnas. Con la restricción adicional
de que solo puede mirar al pasado.
`;
