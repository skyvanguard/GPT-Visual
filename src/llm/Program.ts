...
import { flowParticleSystem } from "./components/FlowParticles";
...
export function runProgram(view: IRenderView, state: IProgramState) {
    ...
    // Update flow particle system
    if (state.display.flowAnimationEnabled && state.layout) {
        flowParticleSystem.setLayout(state.layout);

        // Add flow paths based on model structure
        // Clear and recreate paths each frame for simplicity
        flowParticleSystem.clearPaths();

        // Find key blocks to create flow paths
        const layout = state.layout;
        if (layout.residual0 && layout.tokEmbedObj) {
            // Token embedding to residual
            flowParticleSystem.addPath(
                layout.tokEmbedObj,
                layout.residual0,
                new Vec4(0.3, 0.7, 1.0, 0.8), // Blue particles
                3
            );
        }

        if (layout.posEmbedObj && layout.residual0) {
            // Position embedding to residual
            flowParticleSystem.addPath(
                layout.posEmbedObj,
                layout.residual0,
                new Vec4(0.3, 1.0, 0.7, 0.8), // Cyan particles
                3
            );
        }

        // Add paths through transformer layers if visible
        for (let block of layout.cubes) {
            if (block.t === 'i' && block.opacity > 0.5) {
                // Find connected blocks and create paths
                if (block.deps?.add) {
                    for (const dep of block.deps.add) {
                        if (dep.src.opacity > 0.5) {
                            flowParticleSystem.addPath(
                                dep.src,
                                block,
                                new Vec4(1.0, 0.6, 0.2, 0.6), // Orange particles
                                2
                            );
                        }
                    }
        ...
        flowParticleSystem.update(view.dt / 1000);
        view.markDirty(); // Keep animating while particles are active
    }
