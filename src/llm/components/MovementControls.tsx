...
function constructNavLevels(model: IGptModelLayout) {
    ...
    function makeLayerNorm(name: string, ln: IGptLayerNormLayout): INavLevel {
        let agg: INavLevel = { name: "Agregación LN", block: ln.lnAgg1 };
        let norm: INavLevel = { name: "LN Normalizado", block: ln.lnResid };
        let bias: INavLevel = { name: "Sesgo LN", block: ln.lnSigma };
        let weight: INavLevel = { name: "Peso LN", block: ln.lnMu };
        makeCol([agg, norm]);
        makeRow([weight, bias, norm]);
        return {
            name: "Normalización de Capa",
    ...
    let transformers: INavLevel[] = model.blocks.map((block, i) => {
        ...
        function makeHead(head: IGptModelLayout['blocks'][0]['heads'][0], idx: number) {
            ...
            let qRow: INavLevel[] = [
                { name: "Sesgo Q", block: head.qBiasBlock },
                { name: "Peso Q", block: head.qWeightBlock },
                { name: "Vectores Q", block: head.qBlock },
            ];
            let kRow: INavLevel[] = [
                { name: "Sesgo K", block: head.kBiasBlock },
                { name: "Peso K", block: head.kWeightBlock },
                { name: "Vectores K", block: head.kBlock },
            ];

            let vRow: INavLevel[] = [
                { name: "Sesgo V", block: head.vBiasBlock },
                { name: "Peso V", block: head.vWeightBlock },
                { name: "Vectores V", block: head.vBlock },
            ];
            ...
            let attnSm: INavLevel = { name: "Softmax de Atención", block: head.attnMtxSm };
            let attnAgg1: INavLevel = { name: "Agregación Atención 1", block: head.attnMtxAgg1 };
            let attnAgg2: INavLevel = { name: "Agregación Atención 2", block: head.attnMtxAgg2 };
            let attn: INavLevel = { name: "Matriz de Atención", block: head.attnMtx };
            let attnOut: INavLevel = { name: "Salida de Atención", block: head.vOutBlock };
        ...
        let projection: INavLevel = {
            name: "Proyección",
            children: [
                { name: "Sesgo de Proyección", block: block.projBias },
                { name: "Peso de Proyección", block: block.projWeight },
                { name: "Salida de Proyección", block: block.attnOut },
            ],
        };

        let attnOutput: INavLevel = {
            name: "Salida de Auto-Atención",
        ...
        let fc1: INavLevel = {
            ...
            children: [
                { name: "Sesgo FC", block: block.mlpFcBias },
                { name: "Peso FC", block: block.mlpFcWeight },
                { name: "Salida FC", block: block.mlpFc },
            ],
        };
        let act: INavLevel = {
            name: "Activación MLP",
        ...
        let fc2: INavLevel = {
            name: "Proyección MLP",
            children: [
                { name: "Sesgo FC", block: block.mlpProjBias },
                { name: "Peso FC", block: block.mlpProjWeight },
                { name: "Salida FC", block: block.mlpResult },
            ]
        };
