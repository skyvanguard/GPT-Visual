...
export const TocDiagram: React.FC<{
    ...
    // Neural Insight palette colors
    let colors = {
        ln: '#fbbf24',          // LayerNorm - golden/amber
        multihead: '#fb923c',   // Attention - orange
        feedForward: '#22d3d8', // MLP - turquoise/cyan
        tokEmbed: '#c084fc',    // Embedding - violet
        linear: '#a78bfa',      // Linear - purple
        softmax: '#4ade80',     // Softmax - green
        line: '#334155',        // Lines - slate
        focus: '#818cf8',       // Focus highlight - indigo
    };
    ...
    let structure: IEl = {
        type: ElType.Block, height: 0, color: '#eee', special: 'llm', padX: 2.0, padY: 1.0, id: 'llm', items: [
            { type: ElType.Cell, label: 'emb. token', height: 1, width: 4, color: colors.tokEmbed, id: 'tokEmbed' },
            ...
            { type: ElType.Block, height: 0, padX: 3, padY: 0, color: '#ddd', special: 'transformer', id: 'transformer', items: [
                ...
                { type: ElType.Cell, height: 1, label: 'norm. capa', width: widthStd, color: colors.ln, id: 'ln1' },
                { type: ElType.Gap, height: exitGap, gapType: 'multihead', arrow: true },
                { type: ElType.Cell, height: 2, label: ['multi-cabeza, causal', 'auto-atención'], width: widthMultihead, color: colors.multihead, id: 'selfAttend' },
                { type: ElType.Gap, height: plusGap, gapType: 'add' },
                { type: ElType.Gap, height: exitGap, gapType: 'exit', arrow: true },
                { type: ElType.Cell, height: 1, label: 'norm. capa', width: widthStd, color: colors.ln, id: 'ln2' },
                { type: ElType.Gap, height: smallGap, arrow: true },
                { type: ElType.Cell, height: 2, label: ['red', 'neuronal'], width: widthFeedForward, color: colors.feedForward, id: 'feedForward' },
                { type: ElType.Gap, height: plusGap, gapType: 'add' },
            ] },
            { type: ElType.Gap, height: smallGap, arrow: true },
            { type: ElType.Cell, height: 1, label: 'norm. capa', width: widthStd, color: colors.ln, id: 'lnf' },
            { type: ElType.Gap, height: smallGap, arrow: true },
            { type: ElType.Cell, label: 'lineal', height: 1, width: widthStd, color: colors.linear, id: 'linear' },
            { type: ElType.Gap, height: smallGap, arrow: true },
    ...
    makeEntry(Phase.Input_Detail_LayerNorm, 'Norm. Capa', ['ln1', 'ln2', 'lnf']);
    makeEntry(Phase.Input_Detail_SelfAttention, 'Auto-Atención', ['selfAttend']);
        ...
        switch (el.type) {
            ...
            case ElType.PosEmbed:
                ...
                content = <>
                    ...
                    <text fontSize={11} textAnchor='end' x={textX} y={center.y + 0.25 * fontSize}>emb. posición</text>
    ...
    return <div>
        {titleAbove && <div className={s.tocTitle}>Índice</div>}
