...
export function initBlockRender(ctx: IGLContext | null) {
    ...
    function createFragShader(instanced: boolean) {
        ...
        uniform float u_heatMapIntensity; // 0.0 = off, 1.0 = full intensity
        ...
        vec3 heatMapColor(float value) {
            // Cool to warm color mapping: blue -> cyan -> green -> yellow -> red
            float t = clamp(value, 0.0, 1.0);
            vec3 cold = vec3(0.1, 0.2, 0.6);
            vec3 cool = vec3(0.2, 0.6, 0.9);
            vec3 mid = vec3(0.3, 0.8, 0.3);
            vec3 warm = vec3(0.95, 0.8, 0.2);
            vec3 hot = vec3(0.95, 0.2, 0.1);

            if (t < 0.25) return mix(cold, cool, t * 4.0);
            if (t < 0.5) return mix(cool, mid, (t - 0.25) * 4.0);
            if (t < 0.75) return mix(mid, warm, (t - 0.5) * 4.0);
        ...
        void main() {
            ...
            if (u_accessTexScale > 0.0 && dist < maxDist) { // have access texture
                ...
                if (insideAny) {
                    ...
                    vec3 posColor = u_baseColor.rgb;
                    vec3 zeroColor = vec3(0.5, 0.5, 0.5);

                    // Apply heat map coloring when enabled
                    if (u_heatMapIntensity > 0.0) {
                        float normalizedVal = (val + 1.0) * 0.5; // Map from [-1,1] to [0,1]
                        vec3 heatColor = heatMapColor(normalizedVal);
                        texBaseColor = mix(texBaseColor, heatColor, u_heatMapIntensity * weight);
                    } else {
    ...
    let shader = createShaderProgram(ctx, 'block', createVertShader(false), createFragShader(false),
        ['u_camPos', 'u_accessSampler', 'u_heatMapIntensity'],
...
export function renderAllBlocks(blockRender: IBlockRender, layout: IModelLayout, modelMtx: Mat4f, camPos: Vec3, lightPosArr: Float32Array, lightColorArr: Float32Array, heatMapIntensity: number = 0) {
    let gl = blockRender.gl;
    ...
    gl.uniform1f(locs.u_heatMapIntensity, heatMapIntensity);
...
export function renderAllBlocksInstanced(blockRender: IBlockRender, layout: IModelLayout, modelMtx: Mat4f, camPos: Vec3, heatMapIntensity: number = 0) {
