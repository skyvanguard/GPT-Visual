...
import { IParticleRender, initParticleRender, renderParticles } from "./particleRender";
...
export function initRender(canvasEl: HTMLCanvasElement, fontAtlasData: IFontAtlasData): IRenderState | null {
    ...
    let particleRender = initParticleRender(ctx);
...
export function renderModel(state: IProgramState) {
    ...
    let heatMapIntensity = state.display.heatMapEnabled ? state.display.heatMapIntensity : 0;
    renderAllBlocks(blockRender, layout, modelMtx, camPos, lightPosArr, lightColorArr, heatMapIntensity);
    ...
    for (let example of state.examples) {
        if (example.enabled && example.layout) {
            ...
            renderAllBlocksInstanced(example.blockRender, example.layout, modelMtxLocal, camPos, heatMapIntensity);
        }
    }
    ...
    if (args.particleRender) {
        renderParticles(args.particleRender, flowParticleSystem);
    }
