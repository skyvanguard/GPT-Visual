...
import { BlockTooltipOverlay } from './components/BlockTooltipOverlay';
...
export function LayerView() {
    ...
    let isLoading = !dataAndModel || !canvasRender;
    ...
    let mainView = <div className={s.canvasWrap}>
        <canvas
            ...
            ref={setCanvasEl}
            role="img"
            aria-label="Visualización 3D interactiva de un modelo de lenguaje grande (LLM). Use el ratón para rotar y hacer zoom."
        />
        {isLoading && <div className='absolute flex flex-col items-center w-full h-full justify-center bg-slate-900'>
            <div className={s.spinner}></div>
            <div className='text-white text-lg mt-4'>Cargando modelo...</div>
        </div>}
        {canvasRender && !canvasRender.progState.render && <div className='absolute flex flex-col items-center w-full h-full justify-center'>
            <div className='text-2xl'>Esta aplicación requiere un navegador compatible con WebGL2.</div>
            <div className='text-lg mt-2'>Por favor, prueba la última versión de Chrome o Firefox.</div>
        </div>}
        ...
        {canvasRender && <ProgramStateContext.Provider value={canvasRender.progState}>
            ...
            <MiniMap />
            <div style={{ position: 'fixed', top: 70, right: 20, zIndex: 100 }}>
                <ShareButton />
            </div>
            <SettingsPanel />
            <BlockTooltipOverlay />
